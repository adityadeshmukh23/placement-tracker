export type Language = "en" | "mr";

export type TranslationKey = keyof typeof translations;

export const translations = {
  shop: { en: "Shop", mr: "दुकान" },
  tenant: { en: "Tenant", mr: "भाडेकरू" },
  rent: { en: "Rent", mr: "भाडे" },
  paid: { en: "Paid", mr: "भरले" },
  partial: { en: "Partial", mr: "अंशतः भरले" },
  unpaid: { en: "Unpaid", mr: "थकीत" },
  pending: { en: "Pending", mr: "प्रलंबित" },
  dueDate: { en: "Due Date", mr: "देय तारीख" },
  dueMonth: { en: "Due Month", mr: "देय महिना" },
  totalCollected: { en: "Total Collected", mr: "एकूण जमा" },
  totalPending: { en: "Total Pending", mr: "एकूण प्रलंबित" },
  totalDue: { en: "Total Due", mr: "एकूण देय" },
  addPayment: { en: "Add Payment", mr: "पेमेंट जोडा" },
  addShop: { en: "Add Shop", mr: "दुकान जोडा" },
  addTenant: { en: "Add Tenant", mr: "भाडेकरू जोडा" },
  dashboard: { en: "Dashboard", mr: "डॅशबोर्ड" },
  history: { en: "History", mr: "इतिहास" },
  reports: { en: "Reports", mr: "अहवाल" },
  family: { en: "Family", mr: "कुटुंब" },
  regular: { en: "Regular", mr: "नियमित" },
  area: { en: "Area", mr: "परिसर" },
  address: { en: "Address", mr: "पत्ता" },
  monthlyRent: { en: "Monthly Rent", mr: "मासिक भाडे" },
  phone: { en: "Phone", mr: "फोन" },
  name: { en: "Name", mr: "नाव" },
  amount: { en: "Amount", mr: "रक्कम" },
  paymentMode: { en: "Payment Mode", mr: "पेमेंट पद्धत" },
  cash: { en: "Cash", mr: "रोख" },
  upi: { en: "UPI", mr: "यूपीआय" },
  cheque: { en: "Cheque", mr: "धनादेश" },
  other: { en: "Other", mr: "इतर" },
  notes: { en: "Notes", mr: "टिपा" },
  vacant: { en: "Vacant", mr: "रिकामे" },
  occupied: { en: "Occupied", mr: "भाडेकरू असलेले" },
  save: { en: "Save", mr: "जतन करा" },
  cancel: { en: "Cancel", mr: "रद्द करा" },
  allShops: { en: "All Shops", mr: "सर्व दुकाने" },
  search: { en: "Search", mr: "शोधा" },
  noResults: { en: "No results found", mr: "काही सापडले नाही" },
  back: { en: "Back", mr: "मागे" },
  tenantType: { en: "Tenant Type", mr: "भाडेकरू प्रकार" },
  optional: { en: "optional", mr: "पर्यायी" },
  comingSoon: { en: "Coming soon", mr: "लवकरच येत आहे" },
  date: { en: "Date", mr: "तारीख" },
  addNote: { en: "Add Note", mr: "टीप जोडा" },
  paymentRecorded: { en: "Payment recorded", mr: "पेमेंट नोंदवले" },
  markAsPaid: { en: "Mark as paid", mr: "भरले म्हणून चिन्हांकित करा" },
  thisMonth: { en: "This Month", mr: "हा महिना" },
  missed: { en: "Missed", mr: "चुकले" },
  noTenantCurrently: { en: "No tenant currently", mr: "सध्या भाडेकरू नाही" },
  monthly: { en: "Monthly", mr: "मासिक" },
  yearly: { en: "Yearly", mr: "वार्षिक" },
  export: { en: "Export", mr: "निर्यात करा" },
  month: { en: "Month", mr: "महिना" },
  total: { en: "Total", mr: "एकूण" },
  collected: { en: "Collected", mr: "जमा" },
  installApp: { en: "Install Rental Book for offline use", mr: "ऑफलाइन वापरासाठी Rental Book इंस्टॉल करा" },
  install: { en: "Install", mr: "इंस्टॉल करा" },
  dismiss: { en: "Dismiss", mr: "बंद करा" },
  sendReceipt: { en: "Send Receipt", mr: "पावती पाठवा" },
  sendReminder: { en: "Send Reminder", mr: "स्मरणपत्र पाठवा" },
  noPhoneOnFile: { en: "No phone number on file", mr: "फोन नंबर नोंदवलेला नाही" },
  done: { en: "Done", mr: "पूर्ण झाले" },
  welcomeTitle: { en: "Welcome to Rental Book", mr: "Rental Book मध्ये आपले स्वागत आहे" },
  welcomeBody: {
    en: "Track rent from all your shops in one place. Let's add your first shop to get started.",
    mr: "तुमच्या सर्व दुकानांचे भाडे एकाच ठिकाणी सांभाळा. सुरुवात करण्यासाठी तुमचे पहिले दुकान जोडूया.",
  },
  noShopsYet: { en: "No shops yet", mr: "अजून कोणतेही दुकान नाही" },
  removeTenant: { en: "Remove Tenant", mr: "भाडेकरू काढा" },
  deleteShop: { en: "Delete Shop", mr: "दुकान हटवा" },
  delete: { en: "Delete", mr: "हटवा" },
  areYouSure: { en: "Are you sure?", mr: "तुम्हाला खात्री आहे का?" },
  loading: { en: "Loading…", mr: "लोड होत आहे…" },
  switchLanguage: { en: "Switch language", mr: "भाषा बदला" },
  cloudSync: { en: "Cloud Sync", mr: "क्लाउड सिंक" },
  signIn: { en: "Sign In", mr: "साइन इन" },
  signOut: { en: "Sign Out", mr: "साइन आउट" },
  pin: { en: "PIN", mr: "पिन" },
  enterPin: { en: "Enter shared PIN", mr: "सामायिक पिन टाका" },
  pinGateSubtitle: {
    en: "Enter the household PIN to sync this device with everyone's shared data.",
    mr: "हे डिव्हाइस सर्वांच्या सामायिक डेटाशी सिंक करण्यासाठी घरातील पिन टाका.",
  },
  unlock: { en: "Unlock", mr: "अनलॉक करा" },
  wrongPin: { en: "Wrong PIN. Please try again.", mr: "चुकीचा पिन. कृपया पुन्हा प्रयत्न करा." },
  continueOffline: { en: "Continue without sync", mr: "सिंकशिवाय सुरू ठेवा" },
  signInToSync: { en: "Sign in to sync", mr: "सिंक करण्यासाठी साइन इन करा" },
  syncing: { en: "Syncing…", mr: "सिंक होत आहे…" },
  synced: { en: "Synced", mr: "सिंक झाले" },
  syncOffline: { en: "Offline", mr: "ऑफलाइन" },
  syncError: { en: "Sync failed", mr: "सिंक अयशस्वी" },
  syncNow: { en: "Sync now", mr: "आता सिंक करा" },
  close: { en: "Close", mr: "बंद करा" },
  sendReminders: { en: "Send Reminders", mr: "स्मरणपत्रे पाठवा" },
  regularTenantsPending: { en: "Regular tenants pending", mr: "नियमित भाडेकरू प्रलंबित" },
  familyTenantsPending: { en: "Family tenants pending", mr: "कुटुंब भाडेकरू प्रलंबित" },
  selectAll: { en: "Select all", mr: "सर्व निवडा" },
  sentNext: { en: "Sent — Next", mr: "पाठवले — पुढे" },
  stop: { en: "Stop", mr: "थांबवा" },
  openWhatsApp: { en: "Open WhatsApp", mr: "WhatsApp उघडा" },
  remindersDone: { en: "All done!", mr: "सर्व झाले!" },
  noPendingReminders: {
    en: "No pending shops to remind this month",
    mr: "या महिन्यात स्मरणपत्र देण्यासाठी कोणतेही प्रलंबित दुकान नाही",
  },
  call: { en: "Call", mr: "कॉल करा" },
  share: { en: "Share", mr: "शेअर करा" },
  undo: { en: "Undo", mr: "पूर्ववत करा" },
  alreadyPaidThisMonth: { en: "Already paid this month", mr: "या महिन्यात आधीच भरले आहे" },
  recordAnyway: { en: "Record Anyway", mr: "तरीही नोंदवा" },
  recordingAnywayConfirmed: {
    en: "Recording an additional payment",
    mr: "अतिरिक्त पेमेंट नोंदवत आहे",
  },
  removeThisPayment: {
    en: "Remove this month's payment",
    mr: "या महिन्याचे पेमेंट काढा",
  },
  editMonthlyRent: { en: "Edit Monthly Rent", mr: "मासिक भाडे संपादित करा" },
  shopRent: { en: "Shop Rent", mr: "दुकान भाडे" },
  familyRent: { en: "Family Rent", mr: "कुटुंब भाडे" },
  recordPayment: { en: "Record a Payment", mr: "पेमेंट नोंदवा" },
  addNewShop: { en: "Add New Shop", mr: "नवीन दुकान जोडा" },
  otherRecords: { en: "Other Records", mr: "इतर नोंदी" },
  otherTransaction: { en: "Other Transaction", mr: "इतर व्यवहार" },
  addOtherTransaction: { en: "Add Other Transaction", mr: "इतर व्यवहार जोडा" },
  moneyOut: { en: "Money Out", mr: "गेलेले पैसे" },
  moneyIn: { en: "Money In", mr: "आलेले पैसे" },
  netTotal: { en: "Net", mr: "निव्वळ" },
  category: { en: "Category", mr: "श्रेणी" },
  allCategories: { en: "All categories", mr: "सर्व श्रेण्या" },
  categoryMedical: { en: "Medical", mr: "वैद्यकीय" },
  categoryInsurance: { en: "Insurance", mr: "विमा" },
  categoryDonation: { en: "Donation", mr: "देणगी" },
  categoryPersonal: { en: "Personal", mr: "वैयक्तिक" },
  specifyCategory: { en: "Specify category", mr: "श्रेणी नमूद करा" },
  loggedBy: { en: "Logged By", mr: "नोंदवणारा" },
  fromDate: { en: "From", mr: "पासून" },
  toDate: { en: "To", mr: "पर्यंत" },
  addPhoto: { en: "Add Photo", mr: "फोटो जोडा" },
  removePhoto: { en: "Remove Photo", mr: "फोटो काढा" },
  noOtherTransactions: { en: "No records yet", mr: "अजून कोणतीही नोंद नाही" },
  edit: { en: "Edit", mr: "संपादित करा" },
  insights: { en: "Insights", mr: "आढावा" },
  yearVsLastYear: { en: "This Year vs Last Year", mr: "मागील वर्षाशी तुलना" },
  last12Months: { en: "Last 12 Months", mr: "गेले १२ महिने" },
  mostReliableShops: { en: "Your Most Reliable Shops", mr: "तुमची सर्वात विश्वासार्ह दुकाने" },
  needsAttention: { en: "Shops That Need Attention", mr: "लक्ष देण्याची गरज असलेली दुकाने" },
  incomeByArea: { en: "Where Your Income Comes From", mr: "तुमचे उत्पन्न कुठून येते" },
  thisMonthSoFarHeading: { en: "This Month So Far", mr: "या महिन्यात आतापर्यंत" },
  vacantShopsHeading: { en: "Vacant Shops", mr: "रिकामी दुकाने" },
  noInsightsYet: {
    en: "Add a shop and tenant to start seeing insights here.",
    mr: "इथे आढावा दिसण्यासाठी एखादे दुकान आणि भाडेकरू जोडा.",
  },
  noAttentionNeeded: {
    en: "No shops need attention right now.",
    mr: "सध्या कोणत्याही दुकानाकडे लक्ष देण्याची गरज नाही.",
  },
  navShops: { en: "Shops", mr: "दुकाने" },
  more: { en: "More", mr: "अधिक" },
  viewDetailedReports: { en: "Detailed Reports", mr: "सविस्तर अहवाल" },
  familyDocuments: { en: "Family Documents", mr: "कौटुंबिक कागदपत्रे" },
  addDocument: { en: "Add Document", mr: "कागदपत्र जोडा" },
  documentTitle: { en: "Title", mr: "शीर्षक" },
  belongsTo: { en: "Belongs To", mr: "कोणाचे आहे" },
  categoryIdentity: { en: "Identity", mr: "ओळख" },
  categoryProperty: { en: "Property", mr: "मालमत्ता" },
  categoryFinancial: { en: "Financial", mr: "आर्थिक" },
  categoryCertificates: { en: "Certificates", mr: "प्रमाणपत्रे" },
  categoryVehicle: { en: "Vehicle", mr: "वाहन" },
  ownerFather: { en: "Father", mr: "वडील" },
  ownerMother: { en: "Mother", mr: "आई" },
  ownerFamily: { en: "Family / Shared", mr: "कुटुंब / सामायिक" },
  takePhoto: { en: "Take Photo", mr: "फोटो काढा" },
  chooseFile: { en: "Choose File", mr: "फाईल निवडा" },
  expiryDate: { en: "Expiry Date", mr: "समाप्ती तारीख" },
  sensitiveDocument: { en: "Sensitive Document", mr: "संवेदनशील कागदपत्र" },
  uploadedBy: { en: "Uploaded By", mr: "अपलोड करणारा" },
  uploading: { en: "Uploading…", mr: "अपलोड होत आहे…" },
  uploadFailed: {
    en: "Upload failed. Please check your connection and try again.",
    mr: "अपलोड अयशस्वी. कृपया तुमचे कनेक्शन तपासा आणि पुन्हा प्रयत्न करा.",
  },
  documentsRequireSync: {
    en: "Family Documents requires cloud sync to be set up.",
    mr: "कौटुंबिक कागदपत्रांसाठी क्लाउड सिंक सेट करणे आवश्यक आहे.",
  },
  noDocumentsYet: { en: "No documents yet", mr: "अजून कोणतेही कागदपत्र नाही" },
  documentExpired: { en: "Expired", mr: "मुदत संपली" },
  documentExpiringSoon: { en: "Expiring Soon", mr: "लवकरच मुदत संपणार" },
  specifyOwner: { en: "Specify", mr: "नमूद करा" },
  enterPinToView: { en: "Enter PIN to View", mr: "पाहण्यासाठी पिन टाका" },
  sensitiveDocPinPrompt: {
    en: "This is a sensitive document. Enter the shared PIN to view it.",
    mr: "हे एक संवेदनशील कागदपत्र आहे. ते पाहण्यासाठी सामायिक पिन टाका.",
  },
  viewDownload: { en: "View / Download", mr: "पहा / डाउनलोड करा" },
  hideAmounts: { en: "Hide amounts", mr: "रक्कम लपवा" },
  showAmounts: { en: "Show amounts", mr: "रक्कम दाखवा" },
  openFamilyDocuments: { en: "Open Family Documents", mr: "कौटुंबिक कागदपत्रे उघडा" },
  changePin: { en: "Change PIN", mr: "पिन बदला" },
  currentPin: { en: "Current PIN", mr: "सध्याचा पिन" },
  newPin: { en: "New PIN", mr: "नवीन पिन" },
  confirmNewPin: { en: "Confirm New PIN", mr: "नवीन पिन पुन्हा टाका" },
  pinsDontMatch: { en: "PINs don't match", mr: "पिन जुळत नाहीत" },
  pinTooShort: {
    en: "PIN must be at least 6 characters",
    mr: "पिन किमान ६ अक्षरांचा असावा",
  },
  wrongCurrentPin: { en: "Current PIN is incorrect.", mr: "सध्याचा पिन चुकीचा आहे." },
  pinChanged: { en: "PIN changed successfully", mr: "पिन यशस्वीरित्या बदलला" },
  pinChangedReminder: {
    en: "Remember to tell the rest of the household the new PIN — they'll need to enter it manually on their own devices.",
    mr: "घरातील इतरांना नवीन पिन सांगायला विसरू नका — त्यांना तो त्यांच्या स्वतःच्या डिव्हाइसवर स्वतः टाकावा लागेल.",
  },
  genericChangePinError: {
    en: "Something went wrong. Please try again.",
    mr: "काहीतरी चुकले. कृपया पुन्हा प्रयत्न करा.",
  },
  changePinRequiresSync: {
    en: "Cloud sync isn't set up, so there's no shared PIN to change.",
    mr: "क्लाउड सिंक सेट केलेले नाही, त्यामुळे बदलण्यासाठी कोणताही सामायिक पिन नाही.",
  },
} as const;

export function translate(key: TranslationKey, lang: Language): string {
  return translations[key][lang];
}
